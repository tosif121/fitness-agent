            ...
            if pose_data and "persons" in pose_data and pose_data["persons"]:
                ...
                if len(kpts) >= 17:
                    try:
                        ...
                        # Determine if user is upright vs horizontal
                        is_upright = False
                        if avg_shoulder_y is not None and avg_hip_y is not None:
                            # If shoulders are significantly higher (lower Y) than hips, they are standing/squatting
                            if avg_shoulder_y < avg_hip_y - 40:
                                is_upright = True

                        # --- DETECT PUSH-UPS (Priority #1 if horizontal) ---
                        # Shoulders drop severely relative to wrists
                        if not is_upright and avg_shoulder_y and avg_wrist_y:
                        ...
                        # --- DETECT SQUATS ---
                        if not rep_triggered and is_upright and avg_hip_y and avg_knee_y:
                            # Hips go down near knees
                            if avg_hip_y > avg_knee_y - 40: 
                                self.squat_state = "DOWN"
                            # Hips go up away from knees
                            elif avg_hip_y < avg_knee_y - 90 and self.squat_state == "DOWN":
                                self.squat_state = "UP"
                                count_rep("squats", "good", "Great depth on that squat!")
                                rep_triggered = True

                        # --- DETECT JUMPING JACKS ---
                        if not rep_triggered and is_upright and avg_wrist_y and nose[2] > 0.4:
                            # Wrists go above nose
                            if avg_wrist_y < nose[1]:
                                self.jack_state = "UP"
                            # Wrists go down near hips (or far below nose)
                            elif avg_hip_y and avg_wrist_y > avg_hip_y - 40 and self.jack_state == "UP":
                                self.jack_state = "DOWN"
